...
public class Secteur extends Observable {
	...
	public void predation_reproduction() {
		if (this.vie[1] == 0) // Aucune vie[1]
		{
			...
			if (this.vie[0] + aModif < 3 && this.vie[1] + pModif > 1)// Reproduction
																		// des
																		// prédateurs
			{
			...
			if (this.vie[1] >= 2 && this.vie[1] < 4)
				pModif++; // Reproduction des vie[1]s
	...
	public int deplacementsEspece(int[] tab, int n) {
		...
		for (int i = 0; i < 4; i++) {
			if (tab[i] != 0) {
				switch (tab[i]) {
				case 1:
					// this.world.getSecteur()[x][(y-1+this.world.getHeigth())%this.world.getHeigth()].vie[n]
					// =
					// this.world.getSecteur()[x][(y-1+this.world.getHeigth())%this.world.getHeigth()].vie[n]+1;
